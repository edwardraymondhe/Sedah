...
{
    public static void RandomlyChooseAndSetStartAndExit(MapGrid grid, ref Vector3 startPosition,
                                                        ref Vector3 exitPosition, bool randomPlacement,
    ...
    private static Vector3 RandomlyChoosePositionOnTheEdgeOfTheGrid(MapGrid grid, Vector3 startPosition, Direction direction = Direction.None)
    {
        ...
        switch (direction)
        {
            case Direction.Right:
                do{
                    position = new Vector3(grid.Width - 1, 0, Random.Range(0, grid.Length)); // Largest x val, random value for z
                }while(Vector3.Distance(position, startPosition) <= 3); // No 3x3
                break;
            case Direction.Left:
                do{
                    position = new Vector3(0, 0, Random.Range(0, grid.Length)); // Largest x val, random value for z
                }while(Vector3.Distance(position, startPosition) <= 3); // No 3x3
                break;
            case Direction.Up:
                do{
                    position = new Vector3(Random.Range(0, grid.Width), 0, grid.Length - 1); // Largest x val, random value for z
                }while(Vector3.Distance(position, startPosition) <= 3); // No 3x3
                break;
            case Direction.Down:
                do{
                    position = new Vector3(Random.Range(0, grid.Width), 0, 0); // Largest x val, random value for z
                }while(Vector3.Distance(position, startPosition) <= 3); // No 3x3
                break;
            default:
                break;
